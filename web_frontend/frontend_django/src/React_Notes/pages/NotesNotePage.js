...
let updateNote = async () => {
    await fetch(`${domain}/api/react/notes/${note_id}/`,
    {
        method: 'PUT',
        headers:{
...
let createNote = async () => {
    ...
    await fetch(`${domain}/api/react/notes/`,
    {
        method: 'POST',
        headers:{
            'Content-Type': 'application/json'},
...
useEffect(() => {
    let getNote = async () => {
        ...
        let response = await fetch(`${domain}/api/react/notes/${note_id}/`)
...
{
    return (
        <div className="NA_note">
            <div className="NA_note_header">
                <h3>
            ...
            {note_id !== 'new'?
            <div>
                <input className ="NA_input"   onChange={(t) =>{setNote({...note, 'title':t.target.value})} } value = {note.title} placeholder = 'Insert Title'></input>
                <br></br>
                <textarea onChange={(t) =>{setNote({...note, 'body':t.target.value})} } value = {note.body}></textarea>
            </div>
            :
            <div>
                <input className ="NA_input"   onChange={(t) =>{setNote({...note, 'title':t.target.value})} } placeholder='Insert Title'></input>
                <br></br>
