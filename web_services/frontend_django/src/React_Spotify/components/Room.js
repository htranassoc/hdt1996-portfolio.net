...
export class Room extends Component{
    constructor(props){
        ...
        this.showSettingDetails = () => {
            return (
                <div className="RCS_MiniSettings">
                    <CreateRoomPage roomCode = {this.state.roomCode}/>
                </div>
            )
        }
        this.modifyCreatePage = () => {
            let element = document.getElementById('RCS_Create_Title')
            element.innerHTML='Update Room'
            let update_button = document.getElementById('RCS_update_div')
            console.log(update_button)
            //update_button.classList.remove('RCS_hidden')
        }

        this.renderSettingsButton = () =>{
            return(<div className="RCS_divb1"><button id="RCS_RoomButton1" onClick={() => {this.updateShowSettings(true)}}>Settings</button></div>)
        }
    ...
    render(){
        ...
        {
            return(            
            <div className="RCS_Room">
                
                <div className="RCS_Top_Buttons">
                    {this.state.isHost!==this.debug? this.renderSettingsButton():null}
                    {this.state.showSettings===true?this.showSettingDetails():null}
                    
                    <div className="RCS_divb2"><button id="RCS_RoomButton2" onClick={()=>{this.exitroom()}}>Exit Room</button></div>
                </div>
        ...
        {
            return (
            <div className="RCS_Access_Error">
                <img className="RCS_error_image" src = {error_image} alt=''></img>
